                ...
                <div style="width: 320px; font-family: 'EB Garamond', Georgia, serif; text-align: left; padding: 8px 4px 2px;">
                    ...
                    <div style="display: flex; gap: 8px; align-items: center; margin-top: 12px;">
                        ...
                        <div style="position: relative;">
                            <button onclick="navigator.clipboard.writeText('${venueShareUrl}'); const btn = this; const msg = btn.nextElementSibling; const icon = btn.querySelector('.link-icon'); const origBg = btn.style.background; const origBorder = btn.style.borderColor; btn.style.background = '#618A62'; btn.style.borderColor = '#618A62'; if(icon){ icon.style.backgroundColor = '#ffffff'; icon.style.opacity = '1'; } msg.style.display='block'; setTimeout(() => { msg.style.display='none'; btn.style.background = origBg; btn.style.borderColor = origBorder; if(icon){ icon.style.backgroundColor = 'var(--brand-gold)'; icon.style.opacity = '0.6'; } }, 2000);" style="background: rgba(255,255,255,0.05); border: 1px solid rgba(255,255,255,0.2); border-radius: 50%; width: 42px; height: 42px; cursor: pointer; display: flex; align-items: center; justify-content: center; flex-shrink: 0; transition: all 0.2s ease;" title="Copy direct link to this venue">
                                <div class="link-icon" style="width: 16px; height: 16px; background-color: var(--brand-gold); opacity: 0.6; -webkit-mask-image: url('assets/link.png'); -webkit-mask-size: contain; -webkit-mask-repeat: no-repeat; -webkit-mask-position: center; mask-image: url('assets/link.png'); mask-size: contain; mask-repeat: no-repeat; mask-position: center; transition: all 0.2s ease;"></div>
                            </button>
