...
# I apparently didn't successfully replace setVotingState in the previous attempt because of a mismatch
old_func = '''        window.setVotingState = (stateId) => {
            ...
            if (legendRoundName && legendContext) {
                if (stateId === 'round-1') {
                    ...
                    legendRoundName.innerText = 'Crawl Finished';
                    legendRoundName.style.display = 'block';
                    legendContext.innerText = 'Event Complete';
                }
            }
            
            // Hide all states
            states.forEach(state => {
                const el = this.querySelector(`#state-${state}`);
                if (el) el.style.display = 'none';
            });
            
            // Show active state
            const activeEl = this.querySelector(`#state-${stateId}`);
...
print("setVotingState finally fixed")
